...
<body>
                ...
                <div>
                    ...
                    <nav id="nav-menu" class="hidden absolute py-5 bg-white shadow-lg rounded-lg max-w-[250px] w-full top-full right-4 lg:flex lg:static lg:bg-transparent lg:max-w-full lg:shadow-none lg:rounded-none">
                        <ul class="navUl block lg:flex lg:mt-8 lg:mx-auto">
                            ...
                            <li class="group">
                                <a href="myreview.php" class="text-lg py-2 mx-8">Update / Delete Shoes</a>
                            </li>
    ...
    if (isset($_GET["deleteID"])) {
        ...
        $result = deleteSepatu($deleteID);
        if ($result == 1) {
            echo '<script>window.location.href = "updateDelShoes.php"</script>';
        } else {
                ...
                <div class="container mx-auto px-8 lg:px-4">
                    <div class="flex flex-wrap items-center shadow-xl rounded-lg py-12 lg:py-0">
                        ...
                        <div class="block w-full self-center px-4 lg:px-8 lg:w-1/2">
                            <div class="relative mt-10 lg:mt-0 p-4 bg-white shadow-lg rounded-lg">
                                <?php
                                echo '<h1 class="text-2xl font-semibold mb-4 text-gray-800">' . 'What do you want to do with ' . htmlspecialchars($row['shoes_name'], ENT_QUOTES, 'UTF-8') . '?' . '</h1>';
                                ?>
                                <div class="flex space-x-4">
                                    <form action="shoesUD.php" method="GET">
                                        <input type="hidden" name="deleteID" id="deleteID" value="<?= $shoe["shoes_id"] ?>">
                                        <a href="shoesUD.php?deleteID=<?= $shoe["shoes_id"] ?>">
                                            <button type="button" id="delete" name="delete" class="inline-block px-6 py-2.5 bg-red-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-red-700 hover:shadow-lg focus:bg-red-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-red-800 active:shadow-lg transition duration-150 ease-in-out">Delete</button>
                                        </a>
                                        <input type="hidden" name="updateID" id="updateID" value="<?= $shoe["shoes_id"] ?>">
                                        <a href="shoesUD.php?shoes_id=<?= $shoe["shoes_id"] ?>">
                                            <button type="button" id="delete" name="delete" class="inline-block px-6 py-2.5 bg-red-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-red-700 hover:shadow-lg focus:bg-red-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-red-800 active:shadow-lg transition duration-150 ease-in-out">Delete</button>
                                        </a>
                                    </form>
                                </div>
